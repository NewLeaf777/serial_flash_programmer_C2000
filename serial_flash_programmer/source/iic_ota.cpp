...
namespace
{
	...
	bool lookup_baud(int baudrate, speed_t *speed)
	{
		for (const BaudEntry &entry : kBaudTable)
		{
			if (entry.baud == baudrate)
			{
				*speed = entry.speed;
				return true;
			}
	...
	//*************************************************************************
	int read_byte_timeout(int fd, uint8_t *out, int timeout_ms)
	{
	...
	uint32_t construct_packet(uint8_t *packet, uint16_t command, uint16_t length, const uint8_t *data)
	{
		...
		packet[3] = static_cast<uint8_t>((length & 0xFF00) >> 8);
		packet[4] = static_cast<uint8_t>(command & 0xFF);
		checksum += (command & 0xFF);
		packet[5] = static_cast<uint8_t>((command & 0xFF00) >> 8);
		...
		packet[index++] = static_cast<uint8_t>(checksum & 0xFF);
		packet[index++] = static_cast<uint8_t>((checksum & 0xFF00) >> 8);
		packet[index++] = 0x1B;
		packet[index++] = 0xE4;
		return index;
	}
	...
	//*************************************************************************
	int autobaud_lock(int fd, int timeout_ms)
	{
		flush_port(fd);
	...
	//*************************************************************************
	int send_packet_and_wait_ack(int fd, const uint8_t *packet, uint32_t length, int timeout_ms, bool *nak)
	{
		*nak = false;
	...
	//*************************************************************************
	int read_hex_byte(FILE *fh, uint8_t *out)
	{
	...
	//*************************************************************************
	int recv_ack_checksum_word(int fd, uint16_t *out, int timeout_ms)
	{
		uint8_t lsb, msb;
		...
		*out = static_cast<uint16_t>(lsb | (msb << 8));
		return 0;
	}
	...
	//*************************************************************************
	int download_image_checksum(int fd, FILE *fh, int byte_timeout_ms)
	{
		...
		for (;;)
		{
			...
			unsigned int blockSize = static_cast<unsigned int>(sizeLsb) |
									  (static_cast<unsigned int>(sizeMsb) << 8);
